...
/**************************************************
		...
		**********************************************************/
		...
		$('#YesEm').bind('click',function(){
			...
			$('.variable_price p').text('MYR '+final);
			//alert(final);
		...
		$('#NoEm').bind('click',function(){
			...
			$('#embroidery .bottomFabric span').removeClass('BottomSpanToRemove');
...
function goForCustomize(){
	...
	$('#customize .scrollable ul li').bind('click',function(){
		...
		switch ($('div').data('customize_sleev_checked')){
			case 'hulf_sleev':
					$('#forShirtV').css({'top':'70%','left':'17%','right':'0'});
...
function goForEmbroidery(){
	...
	$('div').data('Embriodery_Yes_or_no','no');
		var selectdSleeve = $('div').data('selected_Sleev');
		//alert(selectdSleeve);
 ...
 success : function( response){
  if(response){
	 ...
	 var priceWithOutSleev = priceWithSleev - 10;
		for(i=1;i<=10;i++){
			var easeIronBase = dataval["fab" + i + "easyiron"];
			var easeIronContrast = dataval["con_fab" + i + "easyiron"];

			if (easeIronContrast == 1) {
				//do nothing
			}else{
				$('#contrastFebric .scrollable ul li').eq(i-1).find('img.easy_iron').hide();
			}

			if(easeIronBase == 1){
				// do nothing
			}else{
				$('#baseFebric .scrollable ul li').eq(i-1).find('img.easy_iron').hide();
			}


	 		//alert("fab" + i + "easyiron == "+easeIronBase+ "Contrast fab" + i + "easyiron == "+easeIronContrast);
		}	
	 
	 $('#c1 .price_sec strong.hulf').text('MYR '+priceWithOutSleev);
	 $('#c1 .price_sec strong.long').text('MYR '+priceWithSleev);
...
//});
	...
	//	$.each(dataval.test,function() { alert(this.keys(0)); });
	 ...
	 for(i=0;i<10;i++){
	 	...
	 	if(colorCodes[i] == undefined){
			
			$('.EmColors li').eq(i).attr('id',colorCodes[i]).css({'display':'none'});
		}
	 }
