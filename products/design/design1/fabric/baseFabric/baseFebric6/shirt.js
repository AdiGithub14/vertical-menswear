...
$('.shirtControls ul li').unbind('click', view);
...
/**************************************************
	...
	$('.steps_bar ul li:not(li#addToCart)').bind('click',showThePopup);
	$('.steps_bar ul li#addToCart').bind('click',showOverallInfo);
	$('.close').bind('click',closeThePopup);
	$('#design ul li').bind('click',changeCustomizeArea);
	...
	-------------------------------------------------------------------------- */
	//$('.steps_bar ul li:gt(0):not(#addToCart)').unbind('click').addClass('disabled');
...
function goForDesign(){
	...
	var dir = "products/design/design1";
...
function goForCustomize(){
	...
	$('#customize .scrollable ul li').bind('click',function(){	
		...
		switch ($('div').data('customize_sleev_checked')){
			case 'hulf_sleev':
					$('#forShirtV').css({'top':'67%','left':'0','right':'0'});
			...
			break;
			case 'full_sleev' :
			$('#forShirtV').css({'top':'67%','left':'0','right':'54%'});
...
function ContrastBasedChanges(){
	...
	function contrastChange(){
				...
				// console.log("dfsdf");
					 ...
					 if(listId == convertedArray[i]){
					 	console.log('This is :'+convertedArray[i] );
					 	$('.shirtHolder ul li').eq(i).find('img').after('<div class="preload"></div> ').hide()
						.attr('src','products/design/'+ preSelectedDesign +'/fabric/contrastFebric/'+selectedContrast+'/'+ convertedArray[i] +'.png').one('load', function() {
                    $(this).fadeIn().next().remove();
                 });
					 }else{
	...
	var totalLength = $('.shirtHolder ul li').length;
		...
		// console.log("dfsdf");
			 ...
			 if(listId == convertedArray[i]){
			 	...
			 	$('.shirtHolder ul li').eq(i).find('img').after('<div class="preload"></div>').hide()
				.attr('src','products/design/'+ designS +'/'+ convertedArray[i] +'.png').one('load', function() {
                    $(this).fadeIn().next().remove();
                 });
...
function showOverallInfo(){
	...
	//$('#closeInfo').css({'cursor':'pointer','Padding':'5px 20px','margin':'10px 0'});
	/*$( "#shirt_viewport" ).append('<div class="appendDiv">Your OverAll Info <br/><br/> Your Design : '
									+$('div').data('Final_design_name')+
									' <br/> Fabric Details <br/> Base Fabric name: '
									+ $('div').data('baseFabric_name') +
									'<br/> Base Fabric code: '+ $('div').data('baseFabric_code') +
									'<br/><br/> Contrast Details <br/> contrast Fabric name: '
									+ $('div').data('contrastFabric_name') +
									'<br/> contrast Fabric code: '
									+ $('div').data('contrastFabric_code') +
									'<br/><br/> Sleev : '
									+ $('div').data('selected_Sleev') +
									' <br/><br/> Embriodery : '
									+ $('div').data('Embriodery_Yes_or_no') +
									'<br/> <button id="closeInfo">Continue to Checkout</button></div>');
			$( ".appendDiv" ).hide().fadeIn(2000);
